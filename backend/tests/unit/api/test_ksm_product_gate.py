...
def test_async_rejected_before_db_or_takeover():
    ...
    with (
        patch("app.api.webhooks._get_notice_store", return_value=store),
        patch("app.api.webhooks.KSMClient") as client,
        patch("app.api.webhooks.make_session") as session,
        patch("app.api.webhooks.run_post_ingest_agents") as agent,
    ):
        client.return_value.get_order_detail.return_value = {
            "billId": "gate-test",
